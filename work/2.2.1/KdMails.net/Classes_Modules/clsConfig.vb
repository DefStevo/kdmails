...
#Region "Deklarationen"
    ...
    Private _strDateiUnreadMails As String = My.Application.Info.DirectoryPath & "\Config\KdMails.Ungelesen.cfg"
    ...
    Private _srUnreadMails As System.IO.StreamReader
    Private _swUnreadMails As System.IO.StreamWriter
    ...
    Public oUnreadMailL As List(Of sUnreadMail)
    Private oUnreadMailZ As sUnreadMail

#End Region

#Region "Funktionen"
    Function InitConfig(ByVal Ordner As Boolean, _
                        ByVal Domänen As Boolean, _
                        ByVal IgnoreList As Boolean, _
                        ByVal Settings As Boolean, _
                        ByVal UnreadMails As Boolean) As Boolean
        ...
        If UnreadMails = True Then
            oUnreadMailL = New List(Of sUnreadMail)
            If ReadConfigUnreadMails() = False Then
                MsgBox("Datei kann nicht gelesen werden " & vbCrLf & _
                       _strDateiUnreadMails, MsgBoxStyle.Critical, "Fehler beim Lesen der Konfiguration")
            End If
        End If
    ...
    Function UnloadConfig() As Boolean
        If _bInitStatus = True Then
            ...
            oSettingsZ = Nothing
            _bInitStatus = False
            oUnreadMailL = Nothing
            oUnreadMailZ = Nothing
            Return True
        End If
    End Function
...
#Region "Datei Funktionen"
    ...
    Private Function ReadConfigUnreadMails() As Boolean
        Dim strBuffer As String = ""

        If System.IO.File.Exists(_strDateiUnreadMails) Then
            _srUnreadMails = New System.IO.StreamReader(_strDateiUnreadMails)
            ...
            _srUnreadMails.Close()

            Return True
        Else
            System.IO.File.Create(_strDateiUnreadMails)
    ...
    Function WriteConfigUnreadMails() As Boolean
        Dim strBuffer As String = ""

        If Not System.IO.File.Exists(_strDateiUnreadMails) Then
            System.IO.File.Create(_strDateiUnreadMails)
        End If

        _swUnreadMails = New System.IO.StreamWriter(_strDateiUnreadMails, False)
        ...
        _swUnreadMails.Write(strBuffer)
        _swUnreadMails.Flush()
        _swUnreadMails.Close()
...
#Region "UnRead Mail Funktionen"
    ...
    Function AddUnreadMail(ByVal Adresse As String, _
        ...
        If Not SearchUnreadMails(EntryId) = -1 Then
            Return False
        End If
    ...
    Function SearchUnreadMails(ByVal EntryID As String) As Integer
    ...
    Function RemoveUnreadMails(Optional ByVal Index As Integer = -1, _
                               Optional ByVal EntryId As String = Nothing) As Boolean

        'Muss anhand der EntryID gesucht werden
        If Index = -1 Then
            Index = SearchUnreadMails(EntryId)
        End If

        oUnreadMailZ = oUnreadMailL(Index)

        'Eintrag zum löschen Markieren
        oUnreadMailZ.bEntfernen = True

        oUnreadMailL(Index) = oUnreadMailZ
